<script lang="ts">
    ...
    const players = data.teams.map((team) => team.players.map(
        player => {
...
<div class="event-list">
        ...
        <div class="event">
            ...
            {#each event.players as player}
                <div class="player">
                    ...
                    style:box-shadow={player.status != "1" ? `0 0 5px 2px ${
                        getStatusColor(player.status)
                    }` : ""}
                    />
                    <span class="player-name">{player.name}</span>
                    <span class="player_health" class:positive={player.health > player.previousHealth} class:negative={player.health < player.previousHealth}>
                    ...
                    {#if player.status != player.previousStatus}
                        <span class="player_status">
                            <span class="previous_status" style:color={getStatusColor(player.previousStatus)}>{getStatus(player.previousStatus)}</span>
                            <span class="arrow">➡</span>
                            <span class="current_status" style:color={getStatusColor(player.status)}>{getStatus(player.status)}</span>
                       </span>
                    {/if}
                </div>
            {/each}
    ...
    <div class="buttons">
        ...
        {:else}
            <button on:click={() => {
                window.location.href = `/games/${data.game.id}/winner`
            }}>Finish</button>
        {/if}
    </div> 
